...
### CocoaPods
Reading List uses various third party libraries, some of which are managed using [CocoaPods](https://cocoapods.org/).CocoaPods focuses on source-based distribution of third party code and automatic integration into Xcode projects. To ensure that CocoaPods is installed, run `bundler install`. To install the libraries, run:
